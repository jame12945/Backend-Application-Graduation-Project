...
const checkEmailError = (req, res, results) => {
  if (results.length > 0) {
    for (let i = 0; i < results.length; i++) {
      if (
        results[i].user_username.toLowerCase() ==
          req.body.user_username.toLowerCase() ||
        results[i].user_email.toLowerCase() ==
          req.body.user_email.toLowerCase() || [
          results[i].user_fname.toLowerCase() ==
            req.body.user_fname.toLowerCase() &&
            results[i].user_lname.toLowerCase() ==
              req.body.user_lname.toLowerCase(),
        ]
      ) {
        return true;
      }
...
app.post("/register", jsonParser, function (req, res, next) {
  connection.execute(
    "SELECT * FROM user_insystem WHERE user_email=? or user_username=? or user_fname=? and user_lname=?",
    [
      ...
      if (users.length == 0) {
        ...
        bcrypt.compare(
          ...
          function (err, isLogin) {
            if (isLogin) {
              var token = jwt.sign(
                { user_id: userId, username: users[0].user_username },
                secret,
                { expiresIn: "12h" }
              );
...
app.put("/newPassword", jsonParser, function (req, res) {
  ...
  const { username } = decode;
  connection.query(
    "SELECT user_password FROM user_insystem WHERE user_username=?",
    [username],
    function (err, users, fields) {
      bcrypt.compare(
        req.body.user_password,
        ...
        function (err, isLogin) {
          if (isLogin) {
            bcrypt.hash(
              req.body.user_newpassword,
              saltRounds,
              function (err, hash) {
                connection.execute(
                  "UPDATE user_insystem SET user_password =? WHERE user_username=? ",
                  [hash, username],
...
app.post("/reserveroom/:roomdetail_id", jsonParser, async function (req, res) {
  try {
    ...
    if (username && user_id) {
      ...
      const roomIsAvailable = await isRoomAvailable(
        roomDetail,
        dateReserve,
        startTime,
        endTime
...
function getRoomDetail(roomDetail) {
  return new Promise((resolve, reject) => {
    connection.execute(
      "SELECT * FROM roomdetail WHERE roomdetail_id=?",
      [roomDetail],
      (err, results, fields) => {
        if (err) {
          reject(err);
...
async function isRoomAvailable(roomDetail, dateReserve, startTime, endTime) {
  ...
  const reservations = await getReservationsForRoomAndDate(
    roomDetail,
    dateReserve,
    startTime,
    endTime
...
function getReservationsForRoomAndDate(
  roomDetail,
  dateReserve,
  startTime,
  endTime
...
app.post("/faceRecognition", jsonParser, async function (req, res) {
  try {
    ...
    const userInSystemData = await getUserIdFromUsername(
    ...
    if (userInSystemData) {
      ...
      if (latestReservation) {
        ...
        res.json({
          status: "ok",
          message:
            "Face recognition successful, and user_id updated in reservation",
        });
      } else {
        res.json({
          status: "error",
          message: "No reservation found with user_id as null",
        });
      }
    } else {
      res.json({
        status: "error",
        message: "No user found with the provided name from face recognition",
      });
    }
  } catch (err) {
    console.log("ข้อผิดพลาด:", err);
...
app.post("/passBookingReserveByFace", jsonParser, async function (req, res) {
  try {
    ...
    const userInSystemData = await getUserIdFromUserfnamelname(
      nameFromFaceRecognition
    );

    console.log(userInSystemData);
    if (userInSystemData) {
      const checkBookingFromUserId = userInSystemData.user_id;
      const resultCompare = await compareBookingWithStartTimeEndTime(
        checkBookingFromUserId
      );
      console.log(resultCompare);
      res.json({
        status: "ok",
...
//เอาเวลาจาก user_id ที่มาถึงณปัจจุบันตรวจสอบว่ามาอยูในช่วง booking ไหน(อันนี้ให้ log ดู)ถ้าพบเจอก็ให้เด้งไปที่หน้ารอเปิดประตูเลย
// ตัวอย่าง get user_id จาก user_username
async function getUserIdFromUserfnamelname(username) {
  return new Promise((resolve, reject) => {
    connection.execute(
      'SELECT * FROM user_insystem WHERE CONCAT(user_fname, " ", user_lname) LIKE ?',
      [`%${username}%`],
...
async function compareBookingWithStartTimeEndTime(checkBookingFromUserId) {
  return new Promise((resolve, reject) => {
    const currentTime = new Date();
    const options = { year: "numeric", month: "2-digit", day: "2-digit" };
    const formattedCurrentDate = currentTime.toLocaleDateString(
    ...
    console.log(currentTimeString);
    connection.execute(
      "SELECT * FROM reservation WHERE user_id = ? AND DATE_FORMAT(date_reservation, '%d/%m/%Y') = ?",
      [checkBookingFromUserId, formattedCurrentDate],
      (err, results, fields) => {
        if (err) {
          reject(err);
        } else {
...
app.get("/selectStatusTime", function (req, res) {
  try {
    ...
    connection.execute(
      "SELECT start_time, end_time FROM reservation WHERE roomdetail_id = 9 AND DATE_FORMAT(date_reservation, '%d/%m/%Y') = ?",
      ...
      function (err, results) {
        ...
        const filteredTimes = results.filter((timeSlot) => {
          return (
            formattedCurrentTime >= timeSlot.start_time &&
            formattedCurrentTime <= timeSlot.end_time
          );
        });
  ...
  try {
    ...
    connection.execute(
      ...
      function (err, results) {
        ...
        // เลือกรายการที่ start_time เป็นตัวแรก
        const finalResult =
          filteredTimes.length > 0 ? filteredTimes.slice(0, 2) : [];
...
app.get("/selectAllRoomBooking", function (req, res) {
  try {
    ...
    connection.execute(
      "SELECT *  FROM reservation WHERE roomdetail_id = 9 AND  DATE_FORMAT(date_reservation, '%d/%m/%Y') >= ?",
      [formattedCurrentDate],
      function (err, result) {
        if (err) {
          res.json({ status: "error", message: err.message });
...
const getUserInfoByName = (name) => {
  return new Promise((resolve, reject) => {
    const query =
      "SELECT user_username, user_id FROM user_insystem WHERE user_username = ?";
...
app.get("/room", (req, res) => {
  connection.query("SELECT * FROM roomdetail", (err, results) => {
    if (err) {
      ...
      res
        .status(500)
        .json({ status: "error", message: "Failed to retrieve room list" });
    } else {
...
function updateRoomStatus(roomdetail_id, status) {
  connection.query(
    "UPDATE roomdetail SET status_room = ? WHERE roomdetail_id = ?",
    [status, roomdetail_id],
    (err) => {
      if (err) {
    ...
    connection.execute(
      ...
      function (err, results) {
        ...
        results.forEach((reservation) => {
          ...
          const date_reservation = moment(
            reservation.date_reservation,
            "YYYY-MM-DD"
          );

          if (
            end_time.isBefore(nowThailand) &&
            date_reservation.isBefore(nowThailand)
          ) {
